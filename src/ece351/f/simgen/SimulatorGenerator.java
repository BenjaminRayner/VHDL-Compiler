...
public final class SimulatorGenerator extends ExprVisitor {
	...
	public void generate(final String fName, final FProgram program, final PrintWriter out) {
		...
		println("Map<String,StringBuilder> output = new LinkedHashMap<String,StringBuilder>();");
		for (AssignmentStatement astmt : program.formulas)
		{
			println("output.put(\"" + astmt.outputVar + "\", new StringBuilder());");
		}
		...
		for (AssignmentStatement astmt : program.formulas)
		{
			println("final String out_" + astmt.outputVar + " = " + generateCall(astmt) + " ? \"1 \" : \"0 \";");
		}

		println("// store outputs");
		for (AssignmentStatement astmt : program.formulas)
		{
			println("output.get(\"" + astmt.outputVar + "\").append(out_" + astmt.outputVar + ");");
		}
		outdent();
		println("}");
		...
		for (AssignmentStatement astmt : program.formulas)
		{
			print("    " + generateSignature(astmt) + " { return ");
			traverseAssignmentStatement(astmt);
			print(";}\n");
		}
