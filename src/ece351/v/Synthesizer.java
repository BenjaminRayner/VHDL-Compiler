...
public final class Synthesizer extends PostOrderExprVisitor {
	...
	private FProgram synthesizeit(final VProgram root) {	
		...
		for (DesignUnit du : root.designUnits) {
			varPrefix = du.identifier;

			for (Statement stmt : du.arch.statements) {

				if (stmt.getClass() == AssignmentStatement.class) {
					AssignmentStatement aStmt = (AssignmentStatement)stmt;
					aStmt = aStmt.varyOutputVar(varPrefix + aStmt.outputVar.identifier);
					aStmt = aStmt.varyExpr(traverseExpr(aStmt.expr));
					result = result.append(aStmt);
				}
				if (stmt.getClass() == Process.class) {
					Process proc = (Process)stmt;
					for (Statement procStmt : proc.sequentialStatements) {

						if (procStmt.getClass() == AssignmentStatement.class) {
							AssignmentStatement aStmt = (AssignmentStatement)procStmt;
							aStmt = aStmt.varyOutputVar(varPrefix + aStmt.outputVar.identifier);
							aStmt = aStmt.varyExpr(traverseExpr(aStmt.expr));
							result = result.append(aStmt);
						}
						else {
							IfElseStatement ifStmt = (IfElseStatement)procStmt;
							FProgram f = implication(ifStmt);
							for (AssignmentStatement astmt : f.formulas) { result = result.append(astmt); }
						}
					}
				}
			}
	...
	private FProgram implication(final IfElseStatement statement) {
		...
		AssignmentStatement output = new AssignmentStatement();

		++condCount;
		cond = cond.varyOutputVar(conditionPrefix + condCount);
		cond = cond.varyExpr(traverseExpr(statement.condition));
		output = output.varyOutputVar(varPrefix + ifb.outputVar);
		output = output.varyExpr(new OrExpr(new AndExpr(cond.outputVar, traverseExpr(ifb.expr)), new AndExpr(new NotExpr(cond.outputVar), traverseExpr(elb.expr))));
