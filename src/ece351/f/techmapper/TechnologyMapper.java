...
public final class TechnologyMapper extends PostOrderExprVisitor {
	...
	public void render(final FProgram program, final Examiner examiner) {
		...
		for (Expr expr1 : allExpr)
		{
			if (substitutions.get(expr1) != null) { continue; }
			for (Expr expr2 : allExpr) {
				if (expr2.isomorphic(expr1)) { substitutions.put(expr2, expr1); }
			}
		}
		...
		for (AssignmentStatement astmt : program.formulas)
		{
			node(astmt.outputVar.serialNumber(), astmt.outputVar.toString());
			edge(substitutions.get(astmt.expr), astmt.outputVar);
		}
	...
	public Expr visitNot(final NotExpr e) {
		final NotExpr e2 = (NotExpr) substitutions.get(e);
		assert e2 != null : "no substitution for " + e + " " + e.serialNumber();
		edge(e2.expr, e2);
		node(e2.serialNumber(), e2.toString(), "../../gates/not_noleads.png");
		return e;
	}

	@Override
	public Expr visitAnd(final AndExpr e) {
		final AndExpr e2 = (AndExpr) substitutions.get(e);
		assert e2 != null : "no substitution for " + e + " " + e.serialNumber();
		edge(e2.left, e2);
		edge(e2.right, e2);
	...
	public Expr visitOr(final OrExpr e) {
		final OrExpr e2 = (OrExpr) substitutions.get(e);
		assert e2 != null : "no substitution for " + e + " " + e.serialNumber();
		edge(e2.left, e2);
		edge(e2.right, e2);
	...
	@Override public Expr visitNaryAnd(final NaryAndExpr e) {
		final NaryAndExpr e2 = (NaryAndExpr) substitutions.get(e);
		assert e2 != null : "no substitution for " + e + " " + e.serialNumber();
		for (Expr children : e2.children) {
			edge(children, e2);
		}
		node(e2.serialNumber(), e2.toString(), "../../gates/and_noleads.png");
		return e;
	...
	@Override public Expr visitNaryOr(final NaryOrExpr e) { 
		final NaryOrExpr e2 = (NaryOrExpr) substitutions.get(e);
		assert e2 != null : "no substitution for " + e + " " + e.serialNumber();
		for (Expr children : e2.children) {
			edge(children, e2);
		}
